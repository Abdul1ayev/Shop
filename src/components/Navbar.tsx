...
export default function Navbar() {
  ...
  return (
    <nav className="flex justify-between h-[110px] items-center p-4 bg-white shadow-md sticky top-0 z-50">
      ...
      <ul
        className={`${
          menuOpen
            ? "flex flex-col absolute left-0 right-0 bg-white items-center top-16 shadow-md p-4 w-full"
            : "hidden"
        } md:flex md:items-center md:space-x-6 md:bg-transparent md:w-auto md:shadow-none md:p-0`}
      >
        {pathname !== "/" && (
          <li>
            <button
              onClick={() => router.push("/")}
              className="border-green-700 border-2 text-green-700 hover:text-green-500 hover:border-green-500 transition-all py-2 px-4 rounded mt-4"
            >
              Home
            </button>
          </li>
              ...
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 bg-white shadow-lg border rounded-md p-2 w-40 z-50">
                  {pathname !== "/cabinet" && (
                    ...
                    >
                      <User size={18} className="mr-2" /> Cabinet
                    </button>
                  )}
                  <button
